...
client.on("message",async function(msg){
	if(msg.author.id != client.user.id && msg.content.toLowerCase().includes("vsauce")){
		...
		await split.forEach(function(value,i){
			//console.log(`[D] ${i}/${split.length}`)
			var a = Math.floor(Math.random() * words.length)
			if(split[i] == words[a]){
				if(Math.floor(Math.random() * 100) >= 50){
					message += " " + split[i];
					for(var x = 0; x < Math.floor(Math.random() * 5); x++){
						message += ` ${((Math.floor(Math.random() * 5) == 4) ? split[i+x] : split[i-x]).replace(new RegExp(words[a],"gm"),"").replace(new RegExp(split[i],"gm"),"")}`
					}
				}
			}
		})

		msg.channel.send(message).catch(() => {
